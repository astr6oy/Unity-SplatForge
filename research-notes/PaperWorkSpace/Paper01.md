...
LLM을 실내 가구 배치에 활용하는 연구도 늘고 있다. LayoutGPT(Feng et al., 2024)는 in-context learning으로 가구 좌표를 CSS 비슷한 명세로 출력하는 방법을 보여주었고, Holodeck(Yang et al., 2024)은 Habitat 시뮬레이터 안에서 주거 공간 전체를 LLM으로 구성하는 데까지 나아갔다.

이 연구들이 공통적으로 보고하는 문제가 있다. LLM은 "침실에는 침대가 있어야 한다"거나 "책상 앞에 의자를 둔다" 같은 상식적 관계는 잘 잡는다. 하지만 좌표의 물리적 타당성은 다른 문제다. 가구가 허공에 뜨거나 벽을 뚫고 나가거나 다른 물체와 겹치는 사례가 빈번하게 보고되었다(Feng et al., 2024; Yang et al., 2024). 근본 원인은 명확한데, LLM은 텍스트 토큰 공간에서 작동하지 유클리드 기하학을 내재적으로 계산하지는 않기 때문이다.
...
확장성 문제도 있다. 현재 파이프라인은 방 하나 단위에 맞춰져 있다. 건물이나 도시 규모로 가려면 3DGS 에셋의 LOD 관리, 스트리밍 로딩, 절두체 기반 선택적 렌더링이 필수적이고, LLM의 추론 범위 역시 복수 방 이상으로 넓혀야 한다. LS-Gaussian(Li et al., 2025) 같은 경량 스트리밍 프레임워크와의 통합이 이 방향의 출발점이 될 수 있다.
...
[2] Mildenhall, B. et al., "NeRF: Representing scenes as neural radiance fields for view synthesis," Commun. ACM, vol. 65, no. 1, pp. 99-106, 2020.
[3] Chen, G. and Wang, W., "A survey on 3D Gaussian splatting," arXiv:2401.03890, 2024.
[4] Zhou, Y. et al., "Evaluating modern approaches in 3D scene reconstruction: NeRF vs Gaussian-based methods," arXiv:2408.04268, 2024.
...
[8] Yi, T. et al., "GaussianDreamer: Fast generation from text to 3D Gaussians by bridging 2D and 3D diffusion models," in Proc. CVPR, 2024.
[9] Tang, J. et al., "LGM: Large multi-view Gaussian model for high-resolution 3D content creation," in Proc. ECCV, 2024.
[10] Feng, W. et al., "LayoutGPT: Compositional visual planning and generation with large language models," in Proc. NeurIPS, vol. 36, 2024.
[11] Yang, Y. et al., "Holodeck: Language guided generation of 3D embodied AI environments," in Proc. CVPR, 2024.
[12] Aras-p, "UnityGaussianSplatting," GitHub repository, 2024. [Online]. Available: https://github.com/aras-p/UnityGaussianSplatting
...
[15] Anthropic, "The Claude 3 model family," Tech. Rep., 2024.
[16] Xie, T. et al., "PhysGaussian: Physics-integrated 3D Gaussians for generative dynamics," in Proc. CVPR, 2024.
[17] Li, X. et al., "No redundancy, no stall: Lightweight streaming 3DGS for real-time rendering," arXiv:2507.21572, 2025.
