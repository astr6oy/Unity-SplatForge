...
**HDRP·URP 양 경로 검증 (2026-05).** 본 한계의 일반성을 평가하기 위해 두 가지 보완 경로를 평행 검증하였다. (a) **선행 연구[56] unity-lab 동일 환경(Unity 6000.3.13f1, HDRP 17.3.0)** 으로 재현한 batchmode + GS PlayMode 캡처 — 결과는 cleared 색상 버퍼(1 distinct color, 분산 0). 이는 본 한계가 특정 Unity 패치 버전(6000.3.6f1)에 국한되지 않고 **HDRP batchmode + GaussianSplatRenderer 구조의 일반적 비호환**임을 확정한다. (b) **URP 17.3.0 (`GS_ENABLE_URP` define)** 으로 분기한 PoC — PlayMode batchmode 에서 GS 단독은 정상 렌더링(228KB PNG, 300+ distinct colors)되어 batchmode + GS 자체가 불가능한 것이 아님을 시사하나, 같은 씬에 mesh 자산이 공존할 때 GaussianSplatURPFeature 의 `Blitter.BlitCameraTexture` composite blit 이 URP 17.3 RenderGraph 의 active color/depth 텍스처와 상호작용하여 mesh 영역을 검정으로 덮어쓰는 별개 통합 한계가 관찰되었다. 즉 본 연구의 자동화 sweep 에 3DGS 시각화를 포함하기 위해서는 (1) HDRP batchmode 의 GS 합성 종결 패치 또는 (2) URP RenderGraph 호환 composite shader 재작성 중 하나가 선행되어야 하며, 두 작업 모두 plugin upstream(aras-p UnityGaussianSplatting) 영역의 구조적 변경이 필요해 본 페이퍼 범위를 초과한다. 본 한계와 우회 시도의 상세 결과는 후속 자료로 보관한다.
