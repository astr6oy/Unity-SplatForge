...
다섯째 한계로 sparse reconstruction 단계의 COLMAP no-CUDA 병목이 있다. 본 연구의 재구성 파이프라인 중 sparse reconstruction 단계는 COLMAP 4.0.3 homebrew 빌드를 사용한다. 해당 빌드는 `Commit Unknown on Unknown without CUDA`로 표기되어 있으며, SIFT feature 추출과 exhaustive matching 전 구간이 CPU에서 실행된다. 결과적으로 macOS M-계열 하드웨어의 Metal GPU와 ANE는 재구성 단계에서 유휴 상태로 남는다. 본 연구의 측정(bonsai 292 photos)에서 SfM 전체 106분 04초 중 **exhaustive_matcher가 88분 43초(83.6%)를 차지**하여, 30K Brush 학습 73분 26초를 상회한다. 즉 macOS 단일 파이프라인의 진짜 속도 병목은 학습이 아니라 SfM 전처리이며, hloc(SuperPoint+SuperGlue) 또는 DUSt3R/MASt3R 같은 matching 단축형 대안 조사가 자연스러운 후속 과제로 남는다.
